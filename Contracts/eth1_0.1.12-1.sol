...
contract Lottery {
   ...
   function deleteContract() public onlyOwner  
   {
    ...
    address internal gasCompensateTo; //Last player activates a contract. More gas will be taken and should be compensated.
    ...
    if (player_count == required_number_players) {
        ...
        gasCompensateTo = msg.sender;
            ...
            if (gamblers_mesh[10].send(bet*4) == true) {
                bet_sum -= bet*4;
                last_winners[1] = lastWinner(gamblers_mesh[10], bet*4);
            }
            else
                 emit EndGame(gamblers_mesh[10], false, 0);
            if (gamblers_mesh[9].send(bet*2) == true) {
                bet_sum -= bet*2;
                last_winners[2] = lastWinner(gamblers_mesh[9], bet*2);
            }
            else
                 emit EndGame(gamblers_mesh[9], false, 0);
            if (gamblers_mesh[8].send(bet*2) == true) {
                bet_sum -= bet*2;
                last_winners[3] = lastWinner(gamblers_mesh[8], bet*2);
            }
            else
                 emit EndGame(gamblers_mesh[8], false, 0);
            if (gamblers_mesh[7].send(bet) == true) {
                bet_sum -= bet;
                last_winners[4] = lastWinner(gamblers_mesh[7], bet);
            }
            else
                 emit EndGame(gamblers_mesh[7], false, 0);
            if (gamblers_mesh[6].send(bet/2) == true) {
                bet_sum -= bet/2;
                last_winners[5] = lastWinner(gamblers_mesh[6], bet/2);
            }
            else
                 emit EndGame(gamblers_mesh[6], false, 0);

            emit EndGame(gamblers_mesh[5], false, 0);
            last_winners[6] = lastWinner(gamblers_mesh[5], 0);
            emit EndGame(gamblers_mesh[4], false, 0);
            last_winners[7] = lastWinner(gamblers_mesh[4], 0);
            emit EndGame(gamblers_mesh[3], false, 0);
            last_winners[8] = lastWinner(gamblers_mesh[3], 0);
            emit EndGame(gamblers_mesh[2], false, 0);
            last_winners[9] = lastWinner(gamblers_mesh[2], 0);
